...
def fetch_twse_stock_year_csv(year_str: str) -> Optional[List[Dict[str, Any]]]:
    """
    抓取台灣證券交易所 (TWSE) 的「上市個股年成交資訊」。
    (CSV 版本 - 根據 API Schema: FMNPTK_ALL)
    
    Args:
        year_str: 年度 (格式: YYYY，例如: 2025)
    """
    
    url = "https://www.twse.com.tw/exchangeReport/FMNPTK_ALL?"
    params = {
    ...
    logging.info(f"[Request] 開始請求: {year_str} (年度) 的「上市個股年成交資訊」CSV 資料")
    ...
    try:
        ...
        if "沒有符合條件的資料" in response.text:
            logging.warning(f"[Data Error] API 回應: 沒有符合條件的資料 (請確認年度格式是否正確)")
            return None
        ...
        # Schema: Year, Code, Name, TradeVolume, TradeValue, Transaction,
        #         HighestPrice, HDate, LowestPrice, LDate, AvgClosingPrice
        field_name_mapping = {
            '年度': 'Year',
            'Year': 'Year',
            '證券代號': 'Code',
            ...
            'LDate': 'LDate',
            '最低價日期': 'LDate',
            '收盤平均價': 'AvgClosingPrice',
            '平均收盤價': 'AvgClosingPrice',
        }
        ...
        for row in reader:
            ...
            # --- 修正點 ---
            # 偵測標頭行（年成交資訊的第一欄通常是「年度」）
            if row[0].strip() == '年度' or row[0].strip() == 'Year':
                header_raw = row
                found_header = True
        ...
        logging.info(f"[Request] ✓ 成功解析 {len(parsed_data)} 筆 (支) 股票年成交資料。")
        return parsed_data
...
if __name__ == "__main__":
    ...
    YEAR_TO_QUERY = '2025'  # 查詢年度 (格式: YYYY，例如: 2025 表示 2025年)
    ...
    logging.info(f"--- 查詢年度: {YEAR_TO_QUERY} ---")
    
    market_data_list = fetch_twse_stock_year_csv(YEAR_TO_QUERY)
    ...
    if market_data_list:
        ...
        print("[提示] 請確認年度格式是否正確 (格式: YYYY，例如: 2025)")
