...
def fetch_twse_stock_trading_volume_csv() -> Optional[List[Dict[str, Any]]]:
    """
    抓取台灣證券交易所 (TWSE) 的「盤後定價交易」。
    (CSV 版本 - 根據 API Schema: BFT41U)
    """
    ...
    logging.info(f"[Request] 開始請求「盤後定價交易」CSV 資料")
    ...
    try:
        ...
        for row in reader:
            ...
            if row[0].strip() == '證券代號' or row[0].strip() == 'Code' or '證券代號' in row[0] or 'Code' in row[0]:
                header_raw = row
                found_header = True
...
if __name__ == "__main__":
    ...
    logging.info(f"--- 開始抓取盤後定價交易資料 ---")
    
    # 調用抓取盤後定價交易資料函數
    market_data_list = fetch_twse_stock_trading_volume_csv()
